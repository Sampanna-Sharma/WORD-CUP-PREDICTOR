...
def getfootdata():
    ...
    for year in ["2010","2014","EURO16"]:
        ...
        for index, row in df.iterrows():
            ...
            temp1 = (ef.as_matrix(columns = ["RATING","SHOOTING" , "DEFENDING"]).reshape(22,3))#.reshape(154,1))
            temp2 = (ff.as_matrix(columns =["RATING","SHOOTING" , "DEFENDING"]).reshape(22,3))#.reshape(154,1))
            #print(temp2)
            tempX = np.append(temp1,temp2).reshape(44,3)
            tempX =  (tempX- tempX.min(axis=0))/(tempX.max(axis=0)-tempX.min(axis=0))
            tempX = tempX.reshape(132,1)
            #print(tempX)
            #print(tempX.shape)
            X.append(tempX)
